...
class ClientTab(ttk.Frame):
    ...
    def refresh(self):
        ...
        for c in Customer.all():
            self.lb.insert(tk.END, c.full_name())
    ...
    def _save(self):
        ...
        if self.selected is None:
            ...
            cust = Customer.all()[self.selected]
            if cust.marker:
                self.map_service.remove_marker(cust.marker)
            cust.update(first, last, city, wash)

        lat, lon = get_coordinates(city)
        cust.marker = self.map_service.add_marker(lat, lon, label=cust.full_name())
    ...
    def _pick(self, _):
        ...
        if not sel: return
        self.selected = sel[0]; c = Customer.all()[self.selected]
        self.e_first.delete(0, tk.END); self.e_first.insert(0, c.first_name)
        self.e_last.delete(0, tk.END);  self.e_last.insert(0, c.last_name)
        self.e_city.delete(0, tk.END);  self.e_city.insert(0, c.city)
        self.cb_wash.set(c.assigned_car_wash)
        self.btn_save.config(text="Zapisz")

    def _delete(self):
        sel = self.lb.curselection()
        if not sel: return
        c = Customer.all().pop(sel[0])
        if c.marker:
            self.map_service.remove_marker(c.marker)
        self.refresh()
        if self.overview_tab: self.overview_tab.refresh()
        self._clear()

    def _clear(self):
        for e in (self.e_first, self.e_last, self.e_city):
            e.delete(0, tk.END)
        self.cb_wash.set("")
