...
class CarWashTab(ttk.Frame):
    def __init__(self, parent, _map_widget):
        ...
        self.selected: int | None = None       # indeks zaznaczonej myjni
    ...
    # ------------------------------------------------------------
    def refresh(self):
        """Przeładowuje Listbox z danymi z rejestru."""
    ...
    def _delete(self):
        ...
        if not sel:
            return
        CarWash.all().pop(sel[0])
        self.refresh()
        self._clear()

    # ------------------------------------------------------------
    def _clear(self):
        for ent in (self.e_name, self.e_city, self.e_type):
            ent.delete(0, tk.END)
